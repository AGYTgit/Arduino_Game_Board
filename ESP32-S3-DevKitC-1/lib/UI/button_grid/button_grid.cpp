...
void Button_Grid::init() {
    ...
    if (this->button_pressed == nullptr) {
        Serial.println("Error: Memory allocation failed for button_pressed");
        return;
    }

    for (uint8_t i = 0; i < (button_grid_dimensions & 0x0F); i++) {
        this->button_pressed[i] = new bool[button_grid_dimensions >> 4];
        if (this->button_pressed[i] == nullptr) {
            Serial.println("Error: Memory allocation failed for button_pressed[" + String(i) + "]");
            return;
        }
    ...
    for (uint8_t i = 0; i < (button_grid_dimensions >> 4); i++) {
        gpio_set_direction(gpio_num_t(i + this->input_pin_offset), GPIO_MODE_INPUT);
    }
    for (uint8_t i = 0; i < (button_grid_dimensions & 0x0F); i++) {
        gpio_set_direction(gpio_num_t(i + this->output_pin_offset), GPIO_MODE_OUTPUT);
        gpio_set_level(gpio_num_t(i + this->output_pin_offset), HIGH);
    }
...
uint8_t Button_Grid::scan(uint8_t scan_delay) {
    for (uint8_t i = 0; i < (button_grid_dimensions >> 4); i++) {
        gpio_set_level(gpio_num_t(i + this->input_pin_offset), HIGH);
        // Serial.println(i + this->input_pin_offset); //
        for (uint8_t ip = 1; ip < (button_grid_dimensions >> 4); ip++) {
            gpio_set_level(gpio_num_t(((i + ip) % (button_grid_dimensions >> 4)) + this->input_pin_offset), LOW);
            // Serial.println(((i + ip) % (button_grid_dimensions >> 4)) + this->input_pin_offset); //
        }

        for (uint8_t j = 0; j < (button_grid_dimensions & 0x0F); j++) {
            gpio_set_level(gpio_num_t(j + this->output_pin_offset), LOW);
            // Serial.println(j + this->output_pin_offset); //
            for (uint8_t jp = 1; jp < (button_grid_dimensions & 0x0F); jp++) {
                gpio_set_level(gpio_num_t(((j + jp) % (button_grid_dimensions & 0x0F)) + this->output_pin_offset), HIGH);
                // Serial.println(((j + jp) % (button_grid_dimensions & 0x0F)) + this->output_pin_offset); //
            }

            if (gpio_get_level(gpio_num_t(i + this->input_pin_offset)) == LOW && this->button_pressed[j][i]) {
                this->button_pressed[j][i] = false;
            } else if (gpio_get_level(gpio_num_t(i + this->input_pin_offset)) != LOW && !this->button_pressed[j][i]) {
